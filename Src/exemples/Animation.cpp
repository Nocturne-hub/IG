...
static float posX = -10.0f;		   // Position x de la sphere
static float posZ = -5.0f;          // Position y de la sphere
static float delta = 0.1f;
static float distanceParcourue = 0.0f;
static float distanceParcourueAngle = 0.0f;
static float longueurTunnel = 4.0 * 10.0 + 2.0 * M_PI * 5.0;
static float longueurTunnel2 = 8.0 * 2.0 * M_PI * 2.0;
...
static void display(void) {
  ...
  if ( obj ) {
	  scene1();
	  int c = distanceParcourue / longueurTunnel * 4.0;
	  float p = distanceParcourue - c*(longueurTunnel / 4.0);
	  glPushMatrix();
	  glRotatef(90.0 * c, 0.0, 1.0, 0.0);
	  if (p <= 10.0) {
		  glTranslatef(10.0, 0.0, 5.0 -p);
	  }else {
		  glTranslatef(5.0,0.0,-5.0);
		  float angle = (p -10.0)/ (5.0 * M_PI / 2) * 90.0;
	...
	scene2();
	int c = distanceParcourue / longueurTunnel2 * 4.0;
	float p = distanceParcourue - c * (longueurTunnel2 / 4.0);
	glPushMatrix();
	float angle = (p - 10.0) / (5.0 * M_PI / 2) * 90.0;

	glutSolidSphere(1.2, 200.0, 200.0);
	glPopMatrix();
...
static void idle(void) {
	...
	if (distanceParcourue >= longueurTunnel)
		distanceParcourue -= longueurTunnel;
